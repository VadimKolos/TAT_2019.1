...
namespace task_DEV2
{
    ...
    public class PhoneticConverter
    {
        ...
        public int indexOfConsonant { get; set; }
        ...
        public PhoneticConverter(string inputedWord)
        {
            ...
            if (inputedWord.Length < 1 || Regex.IsMatch(inputedWord, @"[А-ЯЁ]")
                || Regex.IsMatch(inputedWord, @"[A-Z]") || Regex.IsMatch(inputedWord, @"[a-z]")
                   || Regex.IsMatch(inputedWord, @"[0-9]") || Regex.IsMatch(inputedWord, " ")
                   || countOfPlus > 1 || countOfYo > 1
                   || Regex.IsMatch(inputedWord, @"[!@#$%^&*()_=,.?\/~`<>;:]"))
            {
                this.inputedWord = string.Empty;
                throw new CustomException(message: "Incorrect input");
            }
            else
            {
        ...
        public int ShockVowelSearch(string inputedWord)
        {
            ...
            for (int i = 0; i < inputedWord.Length; i++)
            {
                if (vowels.Contains(inputedWord[i]))
                {
                    countOfVowels++;
                    lastVowelInWord = i;
                }
            ...
            char[] oneLetterWord = inputedWord.ToCharArray();
            if (!inputedWord.Contains("+") && inputedWord.Length > 1 && !inputedWord.Contains("ё") && countOfVowels != 1)
            {
                throw new CustomException(message: "no accent entered");
            }            
            else if (!inputedWord.Contains("+") && inputedWord.Length == 1 && vowels.Contains(oneLetterWord[0]))
            {
                indexOfShockVowel = 0;
            }
            else if (!inputedWord.Contains("+") && inputedWord.Length > 1 && inputedWord.Contains("ё"))
            {
                indexOfShockVowel = inputedWord.IndexOf("ё");
            }
            else if (!inputedWord.Contains("+") && inputedWord.Length > 1 && countOfVowels == 1)
            {
                for (int i = 0; i < inputedWord.Length; i++)
                {
                    if (vowels.Contains(inputedWord[i]))
                    {
                        lastVowelInWord = i;
                        indexOfShockVowel = lastVowelInWord;
                    }
                }
            }
            else
            {
                for (int i = 0; i < inputedWord.Length; i++)
                {
                    if (consonants.Contains(inputedWord[i]))
                    {
                        indexOfConsonant = i;
                        indexOfPlus = inputedWord.IndexOf('+');
                        if (indexOfPlus - 1 == indexOfConsonant || indexOfPlus == 0)
                        {
                            throw new CustomException(message: "emphasis falls on a consonant");
                        }
                        else
                        {
        ...
        public string DoubleSoundVowelsReplace(string inputedWord)
        {
            ...
            for (int i = 1; i < word.Length; i++)
            {
                if (doubleVoicedVowels.Contains(word[i]) && (vowels.Contains(word[i - 1])
                    || diacriticLetters.Contains(word[i - 1])))
                {
